...
def shortest_route_count_forward(xg,yg,vxp,vyp):
    ...
    i,j = return_pos_ghost(xg,yg)
    ip, jp = return_pos_pac(xp, yp, pos_matrix)
    if vxp == 1 and ip+5 <=len(pos_matrix) and pos_matrix[ip+4][jp] != -1:
        ip += 4
       # print(-1)
    elif vxp == -1 and ip-4>=0 and pos_matrix[ip-4][jp] != -1:
        ip -= 4
        #print(0)
    elif vyp == 1 and jp+5 <=len(pos_matrix[0]) and pos_matrix[ip][jp+4] != -1:
        jp += 4
        #print(1)
    elif vyp == -1 and jp-4>=0 and pos_matrix[ip][jp-4] != -1:
        jp -= 4
        #print(2)
    queue = [(i,j)]
    pos_matrix[i][j] =0
...
def router_forward(xg,yg,vxp,vyp):
    ...
    ip,jp = shortest_route_count_forward(xg,yg,vxp,vyp)
    ig,jg = return_pos_ghost(xg,yg)
    path = [(ip,jp)]
    while path[0] != (ig,jg):
       # print(path)
